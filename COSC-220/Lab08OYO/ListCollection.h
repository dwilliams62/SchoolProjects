...
public:
	...
	void insert(int, T);
	void remove(int);
	void postfixCalculator(ListCollection &obj);
};

template<class T>
void ListCollection<T>::postfixCalculator(ListCollection &obj)
{
	cout << "Entering postfix" << endl;
	for (int i = 0; i < obj.size(); i++)
	{
		if (obj.getElement(i) == "+") {
			double var1 = atof(obj.popFront().c_str());
			double var2 = atof(obj.popFront().c_str());
			var1 = var1 + var2;
			obj.pushFront(to_string(var1));
			obj.removeElement("+");
			i = i-2;
		}
		else if (obj.getElement(i) == "-") {
			double var1 = atof(obj.popFront().c_str());
			double var2 = atof(obj.popFront().c_str());
			var1 = var1 - var2;
			obj.pushFront(to_string(var1));
